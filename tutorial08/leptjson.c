...
void lept_copy(lept_value* dst, const lept_value* src)
{
    ...
    case LEPT_OBJECT:
        ...
        for(i = 0; i < src->u.o.size; i++) {
            val = lept_set_object_value(dst, src->u.o.m[i].k, src->u.o.m[i].klen);
...
int lept_is_equal(const lept_value* lhs, const lept_value* rhs)
{
    ...
    case LEPT_OBJECT:
        ...
        for (i = 0; i < lhs->u.o.size; ++i) {
            val = lept_find_object_value((lept_value*)rhs, lhs->u.o.m[i].k, lhs->u.o.m[i].klen);
            if (val == NULL) {
                return 0;
            }
            if (!lept_is_equal(&lhs->u.o.m[i].v, val)) {
                return 0;
            }
        }
        return 1;
...
void lept_erase_array_element(lept_value* v, size_t index, size_t count)
{
    ...
    for(i = index, j = index + count; j < v->u.a.size; i++, j++) {
        lept_move(lept_get_array_element(v, i), lept_get_array_element(v, j));
    }
    for(; i < v->u.a.size; i++) {
...
lept_value* lept_set_object_value(lept_value* v, const char* key, size_t klen)
{
    ...
    lept_member* m = NULL;
    assert(v != NULL && v->type == LEPT_OBJECT && key != NULL);
    val = lept_find_object_value(v, key, klen);
    if (val == NULL) {
        if (v->u.o.size == v->u.o.capacity) {
...
void lept_remove_object_value(lept_value* v, size_t index)
{
    ...
    memcpy(v->u.o.m + index, v->u.o.m + index + 1, sizeof(lept_member) * (v->u.o.size - 1 - index));
